...
//     // Handle predefined deposit amount button clicks
...
//       const amount = customAmount > 0 ? customAmount : parseFloat($('.deposit-amount-btn.active').data('amount'));
...
//       $.ajax({
//         type: 'POST',
//         url: 'http://localhost:5000/api/deposit-amount',
//         headers: {
...
//           toastr.error('An error occurred while submitting the deposit request.');
//         },
//         complete: function () {
...
$(document).ready(function () {
  ...
  $('#depositButton').on('click', function () {
      ...
      // If a predefined amount is selected, use it; otherwise, use the custom amount
      const amount = customAmount > 0 ? customAmount : parseFloat($('.deposit-amount-box.selected').data('amount'));
      ...
      $.ajax({
          ...
          error: function (error) {
              // Handle the error response here
              showMessageModal('An error occurred while submitting the deposit request.', true);
          },
          complete: function () {
...
//         error: function (error) {
//           // Handle the error response here
//           toastr.error('An error occurred while submitting the deposit request.',"Error",toastrOptions);
//         },
//         complete: function () {
//           // Hide loading spinner when the API request is complete
//           hideLoadingSpinner();
...
$(document).ready(function () {
    ...
    function fetchDepositHistory() {
      ...
      $.ajax({
        type: 'GET',
        url: `http://localhost:5000/api/get-deposit-history/${userId}`, 
        headers: {
          'Authorization': `Bearer ${token}`,
        },
        success: function (response) {
          ...
          if (depositHistory.length > 0) {
            ...
            historyContainer.html('<h1 class="text-center">No deposit history available.<h1/>');
          }
        },
        error: function (error) {
          hideLoadingSpinner();
